...
export default function JobSeekerDashboard() {
  ...
  // Initialize with Redux data or defaults
  const [data, setData] = useState({
    ...
    targetRole: profileData?.desiredPosition || "Job Seeker",
    course: profileData?.trainingRecommendations?.[0]?.courseTitle || "No active course",
    courseProgress: 0,
    applications: [
  ...
  // Load latest data on mount/update
  useEffect(() => {
        ...
        try {
          ...
          if (profile) {
            setData(prev => ({
              ...
              targetRole: profile.desiredPosition || prev.targetRole,
            }));
          }
        } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      const updatePayload = {
        ...
        desiredPosition: tempData.targetRole || tempData.desiredPosition,
        skills: tempData.skills || data.skills,
      };

      await saveUserProfile(updatePayload);
